#!/usr/bin/env python3
"""Debug script to step through the rotation logic."""
...
2025-12-01 19:10:02 Job deadbeef-ghi-abc-def: Completed successfully"""
    ...
    try:
        ...
        print(f"Second file size: {os.path.getsize(log_file)}")
        print(f"Position before second parse: {parser.get_current_position()}")
        ...
        entries = parser.parse_new_entries()
        print(f"After reset: {len(entries)} entries")
        if entries:
            print(f"  Job: {entries[0]['job_id']}")
