"""
Test v2 prompt with GPT-4o-mini (121 citations, 3 rounds)
"""
import json
import os
import sys
...
def test_round(citations, round_num):
    """Test one round with GPT-4o-mini (no reasoning_effort parameter)."""
    results = []
    output_file = Path(f'GPT-4o-mini_v2_default_round{round_num}_detailed_121.jsonl')
    ...
    print(f"\n🚀 Testing: GPT-4o-mini + v2 prompt (Round {round_num})", flush=True)
    ...
    print(f"⏱️  Estimated time: ~{(len(citations) - start_idx) * 0.1} minutes (~6 sec/citation)", flush=True)
    print(f"💰 Estimated cost: ~${(len(citations) - start_idx) * 0.001:.2f}", flush=True)
...
def main():
    print("="*70)
    print("FULL TEST: V2 Prompt + GPT-4o-mini (3 Rounds)")
    print("="*70)
    ...
    for round_num in range(1, 4):
        ...
        print(f"\n{'─'*70}")
        print(f"ROUND {round_num} RESULTS: V2 + 4o-mini")
        print(f"{'─'*70}")
    ...
    print(f"\n{'='*70}")
    print(f"FINAL SUMMARY: V2 + 4o-mini (3 Rounds)")
    print(f"{'='*70}")
    for i in range(1, 4):
    ...
    summary_file = Path('v2_4omini_summary.json')
