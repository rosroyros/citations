"""
Test v2 prompt with GPT-5-mini + reasoning_effort=medium (121 citations, 3 rounds)
"""
import json
import os
import sys
...
def test_round(citations, round_num):
    """Test one round with reasoning_effort=medium."""
    results = []
    output_file = Path(f'GPT-5-mini_v2_medium_round{round_num}_detailed_121.jsonl')
...
def main():
    print("="*70)
    print("FULL TEST: V2 Prompt + GPT-5-mini + Medium Reasoning (3 Rounds)")
    print("="*70)
    ...
    for round_num in range(1, 4):
        ...
        print(f"\n{'─'*70}")
        print(f"ROUND {round_num} RESULTS: V2 + 5mini + Medium")
        print(f"{'─'*70}")
    ...
    print(f"\n{'='*70}")
    print(f"FINAL SUMMARY: V2 + 5mini + Medium (3 Rounds)")
    print(f"{'='*70}")
    for i in range(1, 4):
    ...
    summary_file = Path('v2_5mini_medium_summary.json')
