#!/usr/bin/env python3
"""
End-to-end script to generate 1 complete mega guide and build it to HTML.
...
Usage:
    python3 generate_and_build_one_guide.py [guide_number]

    guide_number: 0-14 (default: 0 = "Complete APA 7th Edition Citation Guide")
"""
...
def main():
    """Generate and build one complete guide"""

    # Which guide to generate?
    guide_index = int(sys.argv[1]) if len(sys.argv) > 1 else 0

    logger.info("=" * 80)
    logger.info("END-TO-END GUIDE GENERATION & BUILD")
    logger.info("=" * 80)
    ...
    if guide_index >= len(configs):
        logger.error(f"Invalid guide index: {guide_index} (max: {len(configs)-1})")
        sys.exit(1)

    config = configs[guide_index]

    logger.info(f"\n📘 Generating Guide #{guide_index}: {config['title']}")
    ...
    json_filename = f"mega_guide_{guide_index:02d}_{config['url_slug']}.json"
