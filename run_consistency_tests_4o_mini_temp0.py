#!/usr/bin/env python3
"""
Run consistency tests on GPT-4o-mini with temperature=0 (deterministic).
Tests model 3 times to verify deterministic behavior and measure accuracy.
"""
import json
import os
...
def run_consistency_test(model_name, run_number):
    ...
    output_file = f'Checker_Prompt_Optimization/consistency_test_{model_name}_temp0_run_{run_number}.jsonl'

    print(f"{'='*80}")
    print(f"Running {model_name} (temp=0) - Run {run_number}")
    ...
    for i, item in enumerate(test_set, 1):
        ...
        try:
            # GPT-4o-mini with temperature=0 for deterministic output
...
# Run tests for GPT-4o-mini with temperature=0
model_id = 'gpt-4o-mini'
model_label = 'GPT-4o-mini_optimized_temp0'

print(f"\n{'#'*80}")
print(f"# Testing: {model_label} (temperature=0 for deterministic output)")
print(f"{'#'*80}\n")
...
# Check if results are identical (as expected with temp=0)
if len(model_runs) == 3:
    if model_runs[0]['accuracy'] == model_runs[1]['accuracy'] == model_runs[2]['accuracy']:
        print(f"\n✅ DETERMINISTIC: All 3 runs have identical accuracy (as expected with temp=0)")
    else:
        print(f"\n⚠️  WARNING: Runs have different accuracies despite temp=0!")
...
summary_file = 'Checker_Prompt_Optimization/consistency_test_4o_mini_temp0_summary.json'
with open(summary_file, 'w') as f:
    json.dump(summary, f, indent=2)

print(f"\n✅ GPT-4o-mini (temp=0) tests complete! Summary saved to: {summary_file}")
