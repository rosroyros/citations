#!/usr/bin/env python3
"""
MLA 9 Prompt Baseline Test - Production Configuration

Tests the MLA 9 v1 prompt against the golden test set to establish baseline accuracy.
...
- ⚠️ Iterate if: 65-79% (manual optimization needed)
...
PROMPT_PATH = os.path.join(PROJECT_ROOT, "backend/prompts/validator_prompt_mla9_v1.txt")
OUTPUT_FILE = os.path.join(PROJECT_ROOT, "Checker_Prompt_Optimization/mla9_baseline_results.json")
...
def parse_response(response_text: str) -> List[Dict[str, Any]]:
    """Parse response and extract citations - adapted for MLA 9 output format"""
    results = []
    ...
    for match in matches:
        ...
        # Check for MLA 9 valid marker (different from APA 7)
        is_valid = False
...
async def test_prompt(client, prompt_path: str, citations: List[str], ground_truth: Dict[int, bool], original_data: List[Dict]) -> Dict:
    """Test the MLA 9 prompt against all citations"""
    print(f"\n{'='*60}", flush=True)
    print(f"Testing: MLA 9 v1 Prompt", flush=True)
    ...
    for batch_num, i in enumerate(range(0, len(citations), BATCH_SIZE), 1):
        ...
        for p in parsed:
            if not p.get("correct"):
                ...
                print(f"   ⚠️ #{p['citation_number']}: expected {expected_str}, got {actual_str}", flush=True)
...
async def main():
    print("🧪 MLA 9 Prompt Baseline Test - Production Config", flush=True)
    ...
    # Summary
    print("\n" + "="*60, flush=True)
    print("📊 MLA 9 BASELINE RESULTS", flush=True)
    print("="*60, flush=True)
    ...
    if accuracy >= 80:
        ...
        print(f"⚠️ ITERATE: Accuracy {accuracy:.1f}% in 65-79% range", flush=True)
