#!/usr/bin/env python3
"""
V2: Merge curated seeds with synthetic variants with RANDOM stratified split
"""
...
def stratified_split(entries: List[Dict], train_ratio: float = 0.8, seed: int = 42) -> tuple:
    """
    Random stratified split preserving valid/invalid ratio
    """
    random.seed(seed)
    ...
    random.shuffle(invalid_entries)

    # Split each group
    valid_split_idx = int(len(valid_entries) * train_ratio)
    invalid_split_idx = int(len(invalid_entries) * train_ratio)

    train = valid_entries[:valid_split_idx] + invalid_entries[:invalid_split_idx]
    val = valid_entries[valid_split_idx:] + invalid_entries[invalid_split_idx:]
    ...
    logger.info(f"  Train: {len(train)} ({sum(1 for e in train if e['is_valid'])} valid, {sum(1 for e in train if not e['is_valid'])} invalid)")
    logger.info(f"  Val: {len(val)} ({sum(1 for e in val if e['is_valid'])} valid, {sum(1 for e in val if not e['is_valid'])} invalid)")

    return train, val


def merge_and_clean(seed_file: Path, synthetic_file: Path, output_file: Path):
    """Merge seeds and synthetic variants, deduplicate, and save with stratified split"""
...
if __name__ == "__main__":
    ...
    output_file = base_dir / "final_merged_dataset_v2.jsonl"
