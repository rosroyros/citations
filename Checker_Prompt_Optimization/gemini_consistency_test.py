...
MAX_CONCURRENT = 3 # Max concurrent API calls (1 per run)
...
def parse_response(response_text: str) -> List[Dict[str, Any]]:
    ...
    """
    results = []
    # Regex to find citation blocks starting with CITATION #X
    ...
    for match in matches:
        ...
        is_valid = False
        # Look for the specific success phrase
...
async def call_gemini_safe(model, full_prompt, run_id):
    """
    Wraps the API call with robust retry logic and rate limit handling.
    """
    for attempt in range(MAX_RETRIES):
        try:
            # Generate content - treating everything as a user message
...
async def process_batch(model, prompt_template, batch_citations, start_idx, batch_num, total_batches):
    """
    ...
    # Ensuring it is all in the user prompt as requested
    full_user_message = prompt_template + "\n\n" + format_citations(batch_citations, start_idx)

    tasks = []
    for i in range(RUNS):
        run_id = i + 1
        tasks.append(call_gemini_safe(model, full_user_message, run_id))
...
async def run_consistency_test():
    print(f"🧪 Starting Gemini Consistency Test", flush=True)
    ...
    # Initialize model without system instructions to ensure prompt goes to user block
    ...
    # Structure: citation_number -> { ground_truth: bool, run_1: bool, run_2: bool, run_3: bool, consistent: bool }
    final_stats = {}
    
    # Initialize full output structure
    output_data = {
        "metadata": {
            "model": MODEL_NAME,
            "runs": RUNS,
    ...
    for batch_num, i in enumerate(range(0, len(citations), BATCH_SIZE), 1):
        ...
        batch_results = await process_batch(
            model, prompt_template, batch_citations, start_idx, batch_num, total_batches
        )
    ...
    for c_num in sorted(final_stats.keys()):
        ...
        # Check if we have results for all runs
        results = [stats.get(f"run_{r+1}") for r in range(RUNS)]
        
        # Handle missing data (if parsing failed)
