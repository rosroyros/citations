...
OUTPUT_FILE = "Checker_Prompt_Optimization/gemini_3_flash_preview_consistency_results.json"
    ...
    for attempt in range(MAX_RETRIES):
        try:
            config = types.GenerateContentConfig(
                temperature=1.0, # Standard temp
                max_output_tokens=65536, # High limit to prevent truncation
            )
...
async def process_batch(client, prompt_template, batch_citations, start_idx, batch_num, total_batches):
    ...
    # Note: We might hit rate limits running 3 concurrent requests for the same model
    # but the retry logic should handle it.
...
async def run_consistency_test():
    ...
    for batch_num, i in enumerate(range(0, len(citations), BATCH_SIZE), 1):
        ...
        # A simple heuristic: if we have enough responses in run_1, we skip.
        # This assumes sequential processing.
        # Each batch adds 1 response string per run.
