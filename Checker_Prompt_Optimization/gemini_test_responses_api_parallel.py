...
OUTPUT_FILE = "Checker_Prompt_Optimization/gemini_responses_api_parallel_results.json"

# Updated configuration
BATCH_SIZE = 5  # Start with 5 for initial test
MAX_RETRIES = 3
RETRY_DELAY = 2  # Initial delay in seconds
TEST_LIMIT = 5  # Only test 5 citations for initial run
...
async def test_gemini_model(model_name: str, prompt_template: str, citations: List[str], start_idx: int):
    """Test a single Gemini model with retry logic"""
    print(f"🔵 Testing {model_name} - Citations {start_idx}-{start_idx + len(citations) - 1}", flush=True)
...
async def process_single_batch(prompt_template: str, batch_citations: List[str], start_idx: int, batch_num: int, total_batches: int):
    """Process a single batch across all 4 Gemini models in parallel"""
    ...
    tasks = [
        test_gemini_model(model_name, prompt_template, batch_citations, start_idx)
...
async def run_test(limit: int = TEST_LIMIT):
    ...
    print("🎯 Gemini Models Parallel API Test - Real-time Tracking Edition", flush=True)
    print("=" * 60, flush=True)
    ...
    print(f"🤖 Testing {len(MODELS)} models: {', '.join(MODELS)}", flush=True)
    ...
    print(f"📊 Testing {len(citations)} citations across {len(MODELS)} Gemini models", flush=True)
    ...
    print(f"⚡ Real-time progress tracking enabled", flush=True)
    ...
    results = {
        "metadata": {
            ...
            "test_type": "gemini_parallel_realtime",
    ...
    print("🎉 Real-time tracking test completed successfully!", flush=True)
