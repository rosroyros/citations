...
OUTPUT_FILE = "Checker_Prompt_Optimization/gemini_system_instructions_121_results.json"
...
def load_system_instruction(path: str) -> str:
    print(f"📄 Loading system instruction from {path}", flush=True)
    with open(path, 'r') as f:
        base_instruction = f.read()

    # Add explicit instructions for processing all citations in order
    enhanced_instruction = base_instruction + """

CRITICAL REQUIREMENTS:
- Process ALL citations provided in the content, without exception
- Process citations in the EXACT ORDER they appear in the content
- Number your responses sequentially starting with #1, regardless of what numbers appear in the content
- Do not skip any citations
- Each citation must have a corresponding validation response

EXAMPLE: If you receive 5 citations in content, you must provide responses for CITATION #1, CITATION #2, CITATION #3, CITATION #4, and CITATION #5 in that order."""
...
def parse_response(response_text: str) -> List[Dict[str, Any]]:
    """Enhanced parser based on our known format."""
    results = []
...
async def run_test():
    ...
    print("🎯 Gemini 2.5-Full Flash Test - System Instructions + Default Thinking", flush=True)
    print("=" * 70, flush=True)
    ...
    system_instruction = load_system_instruction(PROMPT_PATH)
    ...
    results = {
        "metadata": {
            ...
            "model": MODEL,
            "test_type": "gemini_system_instructions_default_thinking",
    ...
    print(f"{MODEL.upper()} (System Instructions + Default Thinking):", flush=True)
